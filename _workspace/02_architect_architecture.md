...
- **renderer.js(engine-dev):** `LAYER_ORDERS`에 **15**를 추가한다(`[10,15,20,30,40]`). 순서 = 배경(10) → **terrain-anim(15)** → 엔티티(20) → fx(30) → 캔버스 UI(40). 카메라 셰이크는 `order ≤ 30`에 적용되므로 15는 월드와 함께 흔들린다(의도 — 지형 장식은 필드 일부). 나머지 renderer 계약(§8·§11 DPR·save/restore 격리) 불변.
