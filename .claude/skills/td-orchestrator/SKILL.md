...
description: "타워 디펜스 게임 제작 에이전트 팀(12명)을 조율하는 오케스트레이터. '게임 만들어/제작해/빌드해', '타워 디펜스 개발' 같은 초기 제작 요청은 물론, 후속 작업 — 게임 수정, 버그 수정, 밸런스/난이도 조정, 에셋 재생성/추가/퀄리티 향상, 에셋 3D 변환/고퀄리티화, 스프라이트 시트/애니메이션 적용, 애니메이션 강화(아이들/공격 모션/변신·진화 연출), 애니메이션 자연화(어색함/딱딱함/끊김 개선, 트윈/이징, anime.js/animejs 도입·적용), 특수 공격 이펙트, 타일/지형 다양화·고도화, 타일 색상 일관성/색감 통일/팔레트 보정, UI 개선, 모바일 최적화, GitHub Pages 배포, 이펙트/사운드 보완, 웨이브/맵 추가, 스테이지/레벨 추가, 다단계 스테이지, 점수/스코어/최고기록 시스템, 타워 업그레이드 개편, 다시 실행, 재실행, 업데이트, 보완, 이전 결과 개선, 플레이테스트만 다시, QA만 다시 — 요청 시에도 반드시 이 스킬을 사용할 것. 단순 코드 질문은 직접 응답 가능."
---
...
| 에셋 3D 고퀄리티 전환(스타일 전면 교체) | **복합 업그레이드 경로 필수** — 스타일 전환은 전량 재생성 + 멀티 시퀀스 계약 개정을 동반한다 | qa 에셋 키·아틀라스 정합 + playtester |
| 애니메이션 강화(아이들·공격 모션·진화 변신) | system-architect(시퀀스 계약: idle/attack, 시퀀스 폴백) → asset-artist(멀티 시퀀스 시트) ‖ engine-dev(로더 시퀀스 폴백) → entity-dev(상태 머신·크로스페이드) ‖ fx-dev(변신 연출·시그니처 이펙트) | qa 경계면(시퀀스 키↔draw) + playtester |
| 타일/지형 다양화·고도화 | system-architect(타일 키 패밀리 개정) → asset-artist(타일 패밀리) ‖ map-designer(패밀리 배치·전이 규칙·terrain-anim 배치) ‖ engine-dev(terrain-anim 레이어) + fx-dev(지형 앰비언트) | qa 타일 키 정합 + playtester(건설 판정 시각 명확성) |
| 애니메이션 자연화(트윈·이징, anime.js) | system-architect(벤더링·vis 필드·파사드 API 계약) → engine-dev(vendor 벤더링·export 확인·일시정지 연동) ‖ fx-dev(`src/fx/tween.js` 파사드+연출 전환) ‖ entity-dev(vis 초기화·draw 반영 — update는 vis 불가지) ‖ ui-dev(UI 트랜지션) | qa(sim.mjs exit 0 + import 경계 grep + 헤드리스 스모크) + playtester(체감) |
| 타일 색상 일관성(팔레트 보정) | asset-artist 단독 — td-asset-pipeline §7.5 (앵커 히스토그램 보정 + `--check` 계측, 키·경로 불변이라 계약 개정 불필요) | qa(`--check` 계측표 인용) + playtester(육안 패치워크 확인) |
