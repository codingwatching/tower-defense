...
- **바닐라 JS + ES 모듈만 사용한다.** 빌드 도구(webpack/vite)·프레임워크 금지. 외부 라이브러리도 원칙 금지하되, **단일 파일로 벤더링한 anime.js(트윈 엔진)만 예외** — "절차적 트윈 규약" 참조. 이유: 산출물이 `index.html` 하나로 실행되어야 하고, 여러 에이전트가 빌드 설정 충돌 없이 병렬 작업할 수 있어야 한다. `vendor/`에 커밋된 단일 ESM 파일은 이 조건을 깨지 않는다(CDN import·npm 빌드는 깬다).
...
레이어 번호는 **계약 문서의 화이트리스트에서만** 사용한다. 목록에 없는 번호가 필요하면 코드·주석·완료 보고 어디에도 쓰기 전에 system-architect 승인을 먼저 받는다 — 미승인 번호가 "권장"으로 유통되면 배선 담당이 중재하고 계약이 뒤집히며 전 팀 재작업으로 파급된다(v5 실증: 시체 레이어 15→18→15 왕복이 5개 에이전트 정정 전파를 낳았다).
...
## 절차적 트윈 규약 (v5, anime.js)

스프라이트 시퀀스(프레임 교체)는 포즈만 바꿀 뿐 보간이 없어 4프레임 사이가 딱딱 끊긴다. 프레임 애니메이션 **위에** 절차적 트윈(이징 보간)을 겹쳐 자연스러움을 만든다 — 시트는 "무엇을 하는지"(포즈), 트윈은 "어떻게 움직이는지"(가감속·탄성·오버슈트)를 맡는다.

- **라이브러리·벤더링:** anime.js **v4 ESM**을 `vendor/anime.esm.min.js` 단일 파일로 커밋한다. 파일 첫 줄 주석에 버전 명기. v4는 named export다: `import { animate, createTimeline, stagger, engine } from '../vendor/anime.esm.min.js'`. v3의 default export(`anime({targets:...})`)와 API가 다르므로, 벤더링 직후 `grep -o "export{[^}]*}" vendor/anime.esm.min.js | head -c 300`으로 실제 export를 확인하고 그 형태에 맞춰 쓴다.
- **직접 import 허용 범위: `src/fx/tween.js`(파사드, fx-dev 소유)와 `src/ui/`뿐.** entities/systems/map/data/core는 anime.js를 import하지 않는다. 이유: 헤드리스 게이트(sim.mjs)가 entities/systems를 document 없이 실행한다 — 트윈은 에셋 로더와 같은 "draw 전유" 자산이다(D35-1과 동일 맥락). 게이트: `grep -rl "vendor/anime" src/entities src/systems src/map src/core src/data`가 빈 출력이어야 한다.
- **시각 전용 불변식:** 트윈 대상은 시각 상태뿐이다. 게임플레이 상태(HP·경로 진행도·쿨다운·골드·웨이브 타이머)를 anime.js로 움직이면 고정 타임스텝 결정성이 깨져 밸런스 시뮬과 실플레이가 어긋난다. 게임 로직의 유일한 시계는 update(dt)다.
- **엔티티 시각 상태(vis) 계약:** 엔티티는 생성자에서 `this.vis = { sx:1, sy:1, rot:0, alpha:1, ox:0, oy:0 }`(스케일 x/y·회전 오프셋·투명도·픽셀 오프셋)를 초기화하고 `draw()`는 이를 변환에 반영만 한다. vis 값을 트윈으로 바꾸는 것은 fx(tween 파사드)가 이벤트 구독으로 수행한다 — vis는 fx에 개방된 계약 인터페이스이므로 소유권 위반이 아니다. `update()`는 vis를 읽지도 쓰지도 않는다 → 헤드리스 sim에서 vis는 초기값 그대로라 안전하다. 필드 추가는 system-architect 승인.
- **파사드 API(`src/fx/tween.js`):** `popIn(vis)`(등장), `deathOut(vis, onDone)`(소멸), `punch(vis, scale)`(펀치), `recoil(vis, angle)`(발사 반동), `shake(vis)` 등 연출 프리셋 + `pauseAll()`/`resumeAll()` + `killTweens(vis)`. 다른 모듈은 anime API를 직접 만지지 않고 프리셋을 호출한다 — 이징·지속시간 톤을 한 파일에서 통일해야 화면 전체가 한 손으로 만든 듯 움직인다. UI만 예외적으로 anime 직접 import 허용(DOM 트랜지션은 프리셋으로 일반화가 안 된다).
- **수명 규칙:** 재사용(풀 반환·레벨 교체) 시 `killTweens(vis)`로 잔여 트윈을 끊고 vis를 기본값으로 리셋한다 — 유령 트윈이 남으면 새 개체가 튄다. 사망 연출은 로직 사망(alive=false·보상 지급)과 분리한다: 로직은 즉시, 시체 페이드는 fx 소유의 별도 시각 개체로.
- **일시정지/배속:** main 상태 머신의 일시정지에 `pauseAll()/resumeAll()`을 연동한다. 배속(2x)은 연출 트윈에 비연동 허용 — 0.6초 미만 연출은 배속과 무관해도 어색하지 않고, 연동하면 복잡도만 는다.
- **이징 기본값:** 등장 `outBack`, 소멸 `inQuad`, 펀치·반동 `outElastic`, UI 슬라이드 `outExpo`, 카운트업 `outCubic`. **linear 금지에 준함** — 등속 보간이 바로 "어색한 애니메이션"의 주범이다.
- **성능:** 트윈은 짧게(<0.6s), 이벤트당 1회 생성(매 프레임 생성 금지), 동시 트윈 상한 상수를 파사드에 둔다. 초과 시 가장 오래된 것부터 완료 처리 — 조용한 강등.
